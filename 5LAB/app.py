...
from utils import get_db, get_current_user, login_required, check_rights
...
def user_view(user_id):
    ...
    is_admin = current and current['role_name'] == 'Администратор'
    return render_template('user_view.html', user=user, is_admin=is_admin)
...
def user_edit(user_id):
    current = get_current_user()
    is_admin = current['role_name'] == 'Администратор'
    ...
    if request.method == 'POST':
        ...
        if errors:
            db.close()
            return render_template('user_edit.html', roles=roles, form=form,
                                   errors=errors, user=user, is_admin=is_admin)

        last_name = form.get('last_name', '').strip() or None
        first_name = form['first_name'].strip()
        middle_name = form.get('middle_name', '').strip() or None
        # Non-admins cannot change roles
        role_id = (form.get('role_id') or None) if is_admin else user['role_id']
    ...
    db.close()
    return render_template('user_edit.html', roles=roles, form=form,
                           errors={}, user=user, is_admin=is_admin)
